...
export default function VariancePage() {
  ...
  // We are not using the standard DataTable here because we want row-level color coding
  // and specific formatting for the variance UI.
  
  return (
    <Layout title="Variance Analysis (Wastage / Theft)">
      <div className="mb-6 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl flex gap-3 items-start">
        ...
        <div className="text-sm text-slate-700 dark:text-slate-300">
          ...
          <p>
            ...
            A positive variance means the kitchen used more ingredients than the sales justify (indicating wastage, over-portioning, or theft).
          </p>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-950">
                {rows.length === 0 ? (
                  <tr><td colSpan={8} className="px-6 py-8 text-center text-slate-500">No variance records found for this period. The nightly engine may not have run yet.</td></tr>
                ) : (
                  rows.map((row: any, i: number) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                        ...
                        <td className={`px-6 py-4 whitespace-nowrap text-right font-semibold ${isRed ? 'text-red-600 dark:text-red-400' : isYellow ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                          {row.variance_qty > 0 ? '+' : ''}{row.variance_qty} <span className="text-xs opacity-75">{row.unit}</span>
                        </td>
                        
                        <td className={`px-6 py-4 whitespace-nowrap text-right font-bold ${isRed ? 'text-red-600 dark:text-red-400' : isYellow ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                          {row.variance_pct}%
                        </td>
